import pytest
from ProjectA_Regression.pageObjects.F_7_DriversPage import DriversPage
...
class Fleet_DriversPage_Test(BaseClass):
    ...
    def test_drivers_page_export_driverlist_csv(self):
        log = self.getLogger()
        fleetportal = DriversPage(self.driver)

        log.info("===== Validating Export Driver List CSV =====")
        is_export_csv_status = fleetportal.validate_Drivers_page_download_driverlist_csv_btn(log)

        if is_export_csv_status:
            log.info(" :  ✅ 'Driver List CSV' Export is successful.")
        else:
            log.error(" :  ❌ 'Driver List CSV' Export failed.")
        assert is_export_csv_status, "Driver List CSV should be exportable."

    def test_drivers_page_export_bulk_qrcode(self):
        log = self.getLogger()
        fleetportal = DriversPage(self.driver)

        log.info("===== Validating Export Bulk QR Code =====")
        is_export_bulk_qrcode_status = fleetportal.validate_Drivers_page_export_bulk_qrcode_btn(log)

        if is_export_bulk_qrcode_status:
            log.info(" :  ✅ 'Bulk QR Code' Export is successful.")
        else:
            log.error(" :  ❌ 'Bulk QR Code' Export failed.")
        assert is_export_bulk_qrcode_status, "Bulk QR Code should be exportable."


    def test_drivers_page_download_qrcode(self):
        log = self.getLogger()
        fleetportal = DriversPage(self.driver)

        log.info("===== Validating QR Code Download For Single Driver =====")
        is_qrcode_download_status = fleetportal.validate_Drivers_page_download_qrcode_btn(log)

        if is_qrcode_download_status:
            log.info(" :  ✅ QR Code Download is successful for single driver.")
        else:
            log.error(" :  ❌ QR Code Download failed for single driver.")
        assert is_qrcode_download_status, "QR Code should be downloadable for single driver."
