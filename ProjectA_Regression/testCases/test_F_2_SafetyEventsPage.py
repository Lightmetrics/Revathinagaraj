...
from ProjectA_Regression.pageObjects.F_2_SafetyEventsPage import SafetyEventsPage
...
class Fleet_SafetyEventsPage_Test(BaseClass):
    ...
    def test_safety_events_navigation_to_tripdetails(self):
        log = self.getLogger()
        fleetportal = SafetyEventsPage(self.driver)

        log.info("===== Validating Navigation to Trip Details page =====")
        navigation_status = fleetportal.validate_safety_events_navigation_to_tripdetails(log)

        if navigation_status:
            log.info(" :  ✅ Successfully navigated to Trip Details page.")
        else:
            log.error(" :  ❌ Failed to navigate to Trip Details page.")
        assert navigation_status, "Navigation to Trip Details page should be successful."
