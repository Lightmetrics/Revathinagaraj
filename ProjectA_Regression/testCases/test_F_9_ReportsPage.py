import pytest
from ProjectA_Regression.pageObjects.F_9_ReportsPage import ReportsPage
...
class Fleet_ReportsPage_Test(BaseClass):
    ...
    def test_reports_page_video_request_list_report(self):
        log = self.getLogger()
        reports_page = ReportsPage(self.driver)

        log.info("===== Validating Video Request List Report presence =====")
        video_request_list_report_status = reports_page.validate_reports_page_video_request_list_report(log)

        if video_request_list_report_status:
            log.info(" :  ✅ Video Request List Report is present.")
        else:
            log.error(" :  ❌ Video Request List Report is NOT present.")
        assert video_request_list_report_status, "Video Request List Report should be present."
    ...
    def test_reports_page_highg_event_list_report(self):
        log = self.getLogger()
        reports_page = ReportsPage(self.driver)

        log.info("===== Validating High G Event List Report presence =====")
        high_g_event_list_report_status = reports_page.validate_reports_page_highg_event_list_report(log)

        if high_g_event_list_report_status:
            log.info(" :  ✅ High G Event List Report is present.")
        else:
            log.error(" :  ❌ High G Event List Report is NOT present.")
        assert high_g_event_list_report_status, "High G Event List Report should be present."

    def test_reports_page_panic_button_list_report(self):
        log = self.getLogger()
        reports_page = ReportsPage(self.driver)

        log.info("===== Validating Panic Button List Report presence =====")
        panic_button_list_report_status = reports_page.validate_reports_page_panic_button_list_report(log)

        if panic_button_list_report_status:
            log.info(" :  ✅ Panic Button List Report is present.")
        else:
            log.error(" :  ❌ Panic Button List Report is NOT present.")
        assert panic_button_list_report_status, "Panic Button List Report should be present."
