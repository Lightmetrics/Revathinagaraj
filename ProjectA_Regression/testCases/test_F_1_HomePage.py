...
from ProjectA_Regression.pageObjects.F_1_HomePage import Fleet_HomePage
...
class Fleet_HomePage_Test(BaseClass):
    ...
    def test_home_page_validate_video(self):
        log = self.getLogger()
        fleet = Fleet_HomePage(self.driver)

        log.info(" : ===== Validating Inside Video =====")
        try:
            assert fleet.validate_video_name(log), "Inside Video not displayed"
            log.info(" :  ✅ Inside Video is displayed.")
        except Exception as e:
            log.error(f" :  ❌ Video validation failed: {e}")
            raise

    def test_home_page_trip_details_btn(self):
        log = self.getLogger()
        fleet = Fleet_HomePage(self.driver)

        log.info(" : ===== Validating Navigating To Trip Details Page =====")
        try:
            assert fleet.validate_navigating_to_trip_details_page(log), "Trip details button not working/displayed"
            log.info(" :  ✅ Trip details button is functional and displayed.")
        except Exception as e:
            log.error(f" :  ❌ Trip details button validation failed: {e}")
            raise
